    ...
    if mode == "predict":
        ...
        while True:
            ...
            if image is None:
                ...
                r_image = yoloFace.detect_image(image)
                r_image = cv2.cvtColor(r_image,cv2.COLOR_RGB2BGR)
                cv2.imwrite('./predict/'+name,r_image)
                print(f'predict image {img} save in ./predict/{name}')
        ...
        while(True):
            ...
            # 进行检测
            frame = np.array(yoloFace.detect_image(frame))
        ...
        tact_time = yoloFace.get_FPS(img, test_interval)
